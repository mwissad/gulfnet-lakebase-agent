...
async def _queue_worker_loop():
    """Background dequeuer for Lakebase task queue (demo-scale)."""
    import asyncio

    from agent_server.orchestration import process_one_task
    ...
    while True:
        ...
        await asyncio.sleep(float(os.getenv("QUEUE_POLL_SECONDS", "3")))
...
async def _lifespan(app):
    ...
    try:
        async with lakebase_context(LAKEBASE_CONFIG) as (checkpointer, store):
            ...
            if os.getenv("ENABLE_QUEUE_WORKER", "1") == "1":
                _worker_task = asyncio.create_task(_queue_worker_loop())
                logger.info("GulfNet queue worker started")
